...
logging.basicConfig(filename="gamma_DCE_sigmoid_gamma_beta_map_ffd_last.log", filemode="a", format="%(levelname)s: %(message)s",level=logging.INFO)
...
def lowlight(image_path, label_path,gamma_net,ffd_model):
    ...
    sigma = torch.full((1,1,1,1), 15/255.).type_as(enhanced_image)
...
def test_lowlightimage(gamma_net,config,epoch,ffd_model):
    with torch.no_grad():
        sum_psnr = 0
        sum_ssim = 0
        test_size = 0
        ...
        for image in test_list:
            ...
            psnr, ssim = lowlight(image, image_label,gamma_net,ffd_model)
            test_size += 1
            sum_psnr += psnr
            sum_ssim += ssim
...
def train(config):
    ...
    nc = 96
    nb = 12
    model_path = config.pretrain_ffd_dir
    ...
    for epoch in range(config.num_epochs):
        for iteration, img_lowlight in enumerate(train_loader):
            ...
            Loss_TV = 1600*(L_TV(gamma) + L_TV(beta))
        ...
        test_lowlightimage(gamma_net,config,epoch,ffd_model) 		
...
if __name__ == "__main__":
    ...
    parser.add_argument('--snapshots_folder', type=str, default="E:/python/Zero-DCE-master/Zero-DCE_code/snapshot_gamma_with_gamma_beta_map_ffd_last/")
    ...
    parser.add_argument('--pretrain_ffd_dir', type=str, default= r"E:\python\Zero-DCE-master\Zero-DCE_code\FFD_net_code\ffdnet_color.pth")
